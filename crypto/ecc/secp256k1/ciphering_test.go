...
func TestEncrypt(t *testing.T) {
	in := []byte("121213kl1l2k3l12k31")
	key, err := ecdsa.GenerateKey(elliptic.P256(), rand.Reader)
	if err != nil {
		t.Fatalf("err %s", err)
	}
	var priKey = PrivateKey(*key)
	out, err := Encrypt(priKey.PubKey(), in)
	if err != nil {
		t.Fatalf("encrypt failed! %s", err)
	}
	t.Log("encrypted is ", string(out))
	deOut, err := Decrypt(&priKey, out)
	if err != nil {
		t.Fatalf("decrypt failed! %s", err)
	}
	t.Log("encrypted is ", string(deOut))
	if string(deOut) != string(in) {
		t.Fatalf("decrypt not same")
	}
